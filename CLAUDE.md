...
| 配布形式             | macOS: `.dmg`, Windows: `.exe` (Inno Setup)           |
| コード署名           | ad-hoc（署名なし）                                    |
| 配布先               | GitHub Releases                                       |
...
- `coderm.quickOpen.excludeTerminals` (boolean, default: `false`)
...
全機能・インフラissueは GitHub Projects (project/1) で管理:

- #1-#7: 機能実装（上記7機能）
- #8: CI/CDパイプライン（macOS/Windowsビルド）
- #9: upstream月次追従ワークフロー
- #10: product.json ブランディング変更
