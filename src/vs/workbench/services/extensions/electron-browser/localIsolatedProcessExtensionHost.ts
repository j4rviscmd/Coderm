...
// --- Coderm file: Phase 6 isolated language extension host. Cloned from
// localProcessExtensionHost.ts; routes a configurable set of language extensions
// into a separate utility process so their crashes cannot take down the main EH. ---
...
import { LocalIsolatedProcessRunningLocation } from '../common/extensionRunningLocation.js';
...
export interface ILocalIsolatedProcessExtensionHostInitData {
	readonly extensions: ExtensionHostExtensions;
}

export interface ILocalIsolatedProcessExtensionHostDataProvider {
	getInitData(): Promise<ILocalIsolatedProcessExtensionHostInitData>;
}
...
export class NativeLocalIsolatedProcessExtensionHost extends Disposable implements IExtensionHost {
	...
	constructor(
		public readonly runningLocation: LocalIsolatedProcessRunningLocation,
		public readonly startup: ExtensionHostStartup.EagerAutoStart | ExtensionHostStartup.EagerManualStart,
		private readonly _initDataProvider: ILocalIsolatedProcessExtensionHostDataProvider,
	...
	private async _start(): Promise<IMessagePassingProtocol> {
		...
		const opts: IExtensionHostProcessOptions = {
			...
			// Distinguished so the main-side ExtensionHostStarter names the spawned
			// utility process 'isolated-extension-host' (visible in Process Explorer).
			kind: 'isolatedExtensionHost',
			// --- Coderm end ---
			responseWindowId: this._nativeHostService.windowId,
			responseChannel: 'coderm:startIsolatedExtensionHostMessagePortResult',
		...
		if (!this._environmentService.isBuilt && !this._environmentService.remoteAuthority || this._isExtensionDevHost) {
			startupTimeoutHandle = setTimeout(() => {
				this._logService.error(`[IsolatedExtensionHost]: Extension host did not start in 10 seconds (debugBrk: ${this._isExtensionDevDebugBrk})`);
	...
	private _establishProtocol(extensionHostProcess: ExtensionHostProcess, opts: IExtensionHostProcessOptions): Promise<IMessagePassingProtocol> {
		...
		return new Promise<IMessagePassingProtocol>((resolve, reject) => {
			...
			const handle = setTimeout(() => {
				reject('The isolated extension host took longer than 60s to connect.');
			}, 60 * 1000);
			...
			extensionHostProcess.start(opts).then(({ pid }) => {
				...
				this._logService.info(`Started isolated extension host with pid ${pid}.`);
		...
		return new Promise<void>((resolve, reject) => {
			...
			const installTimeoutCheck = () => {
				timeoutHandle = setTimeout(() => {
					reject('The isolated extension host took longer than 60s to send its ready message.');
				}, 60 * 1000);
			};
